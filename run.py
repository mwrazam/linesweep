...
# For point search, use KD- tree , look in computational geometry book
...
def check_intersection(projection_line, all_segs):
    for S_idx in range(len(all_segs) - 1):
        ...
        if intersection:
            ...
            print(" FOUND AN INTERSECTION OH NO")
            return True
    return False
...
def projection_by_segment_walk(ref_line, proj_line):
    ...
    for P in proj_line:
        for S_idx in range(current_S, len(ref_line) - 1):
            ...
            #print(f"projection: {projection}, projected point: {projected_point}, magnitude of S: {magnitude_S}")
            ...
            if projection < 0: # The point is before this segment, go to next point, stay on the same segment
                ...
                any_intersections = check_intersection([P,S1], proj_line)
                ...
                #print(f"...negative projection, projecting to point {S1}")
